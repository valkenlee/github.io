const TRANSLATIONS = {
    ko: {
        ...
        bestReport: {
            ...
            yakumanTenpaiAlert: "역만 텐파이 상태입니다! ({yaku})",
        ...
        // 🀄 버림패 분석 리포트 (discardReport)
        discardReport: {
            title: "버림패별 텐파이 및 역만 효율 분석 리포트",
            tileCount: "{count}장",
            tagMaxWait: "최다 대기패",
            tagYakumanTenpai: "✨ 역만 텐파이 ({yaku})",
            discard: "버림",
            waits: "대기패",
            totalCount: "총 {count}장",
            noten: "노텐 (텐파이 불가)",
            tipYakumanOnly: "화료율은 다소 낮지만 역만을 노릴 수 있는 버림패입니다. ({yaku})",
            tipBestAndYakuman: "대기패 수량도 최다이며 역만까지 노릴 수 있는 최고의 버림패입니다! ({yaku})"
        },

        // 🀄 버림패 판정 결과 (discardResult)
        discardResult: {
            case1Title: "[정답] 완벽한 선택! (최다 대기 & 역만)",
            case1Desc: "선택하신 [{tile}]번 패를 버리면 가장 많은 대기패(<b>{count}장</b>)와 함께 역만 텐파이를 완성합니다!",
            case2Title: "[정답] 최적 대기패 선택!",
            case2Desc: "선택하신 [{tile}]번 패를 버리면 가장 많은 대기패(<b>{count}장</b>)를 얻어 화료율이 가장 높습니다.",
            case3Title: "[정답] 화료율 중심 선택! (역만 포기)",
            case3Desc: "역만 기회를 양보하고, 대기패 수량이 가장 많은 [{tile}]번 패(<b>{count}장</b>)를 선택하여 승률을 극대화했습니다.",
            case4Title: "[정답] 역만 노림수 인정!",
            case4Desc: "대기패 수량은 최다가 아니지만, 한 판 역전을 노릴 수 있는 역만 텐파이 버림패인 [{tile}]번 패를 선택하셨습니다!",
            case5Desc: "선택하신 [{tile}]번 패는 대기패 매수가 최다가 아니거나 텐파이가 되지 않습니다.",
            recMaxWait: "가장 많은 대기패를 얻는 버림패: <b>[ {tiles} ]</b> (총 {count}장)",
            recYakuman: "역만을 노릴 수 있는 버림패: <b>[ {tiles} ]</b>"
        },

        alertSelectDiscardTile: "버릴 패를 하나 선택해 주세요.",
    ...
    ja: {
        ...
        bestReport: {
            ...
            yakumanTenpaiAlert: "役満テンパイ状態です！ ({yaku})",
            optimalChoice: "[最適な選択]",
        ...
        discardReport: {
            title: "打牌別テンパイおよび役満効率分析レポート",
            tileCount: "{count}枚",
            tagMaxWait: "最多受け入れ",
            tagYakumanTenpai: "✨ 役満テンパイ ({yaku})",
            discard: "切牌",
            waits: "待ち牌",
            totalCount: "計 {count}枚",
            noten: "ノーテン (テンパイ不可)",
            tipYakumanOnly: "和了率は少し下がりますが、役満が狙える切牌です。 ({yaku})",
            tipBestAndYakuman: "受け入れ枚数も最多で役満まで狙える最高の切牌です！ ({yaku})"
        },

        discardResult: {
            case1Title: "[正解] 完璧な選択！ (最多受け入れ & 役満)",
            case1Desc: "選択した [{tile}] 番の牌を切ると、最大の受け入れ(<b>{count}枚</b>)と同時に役満テンパイが完成します！",
            case2Title: "[正解] 最適な切牌選択！",
            case2Desc: "選択した [{tile}] 番の牌を切ると、最も多くの待ち牌(<b>{count}枚</b>)が得られ、和了率が最も高くなります。",
            case3Title: "[正解] 和了率重視の選択！ (役満回避)",
            case3Desc: "役満チャンスを譲り、最も受け入れ枚数が多い [{tile}] 番の牌(<b>{count}枚</b>)を選択して勝率を最大化しました。",
            case4Title: "[正解] 役満狙いを認定！",
            case4Desc: "受け入れ枚数は最多ではありませんが、逆転が狙える役満テンパイの切牌である [{tile}] 番を選択しました！",
            case5Desc: "選択した [{tile}] 番の牌は受け入れ枚数が最多ではないか、テンパイになりません。",
            recMaxWait: "最も多くの待ち牌が得られる切牌: <b>[ {tiles} ]</b> (計 {count}枚)",
            recYakuman: "役満が狙える切牌: <b>[ {tiles} ]</b>"
        },

        alertSelectDiscardTile: "捨てる牌を1つ選択してください。",
    ...
    zh_CN: {
        ...
        bestReport: {
            ...
            yakumanTenpaiAlert: "已达成役满听牌！({yaku})",
            optimalChoice: "[最佳选择]",
        ...
        discardReport: {
            ...
            tileCount: "{count}张",
            tagMaxWait: "最多听牌",
            tagYakumanTenpai: "✨ 役满听牌 ({yaku})",
            discard: "打出",
            waits: "听牌",
            totalCount: "共 {count}张",
            noten: "未听牌 (无法听牌)",
            tipYakumanOnly: "虽然和牌率稍低，但可以冲击役满的打牌。 ({yaku})",
            tipBestAndYakuman: "听牌张数最多且能冲击役满的最佳打牌！ ({yaku})"
        },

        discardResult: {
            case1Title: "[正确] 完美选择！ (最多听牌 & 役满)",
            case1Desc: "打出您选择的 [{tile}] 号牌，可在获得最多听牌张数(<b>{count}张</b>)的同时达成役满听牌！",
            case2Title: "[正确] 最佳听牌选择！",
            case2Desc: "打出您选择的 [{tile}] 号牌可获得最多的听牌张数(<b>{count}张</b>)，和牌率最高。",
            case3Title: "[正确] 胜率优先选择！ (放弃役满)",
            case3Desc: "放弃役满机会，选择听牌张数最多的 [{tile}] 号牌(<b>{count}张</b>)，使胜率最大化。",
            case4Title: "[正确] 认可役满尝试！",
            case4Desc: "虽然听牌张数不是最多，但您选择了有机会一波逆转的役满听牌打牌 [{tile}] 号牌！",
            case5Desc: "您选择的 [{tile}] 号牌听牌张数不是最多，或无法达成听牌。",
            recMaxWait: "获得最多听牌的打牌: <b>[ {tiles} ]</b> (共 {count}张)",
            recYakuman: "可冲击役满的打牌: <b>[ {tiles} ]</b>"
        },

        alertSelectDiscardTile: "请选择一张要打出的牌。",
    ...
    zh_TW: {
        ...
        bestReport: {
            ...
            yakumanTenpaiAlert: "已達成役滿聽牌！({yaku})",
            optimalChoice: "[最佳選擇]",
        ...
        discardReport: {
            ...
            tileCount: "{count}張",
            tagMaxWait: "最多聽牌",
            tagYakumanTenpai: "✨ 役滿聽牌 ({yaku})",
            discard: "打出",
            waits: "聽牌",
            totalCount: "共 {count}張",
            noten: "未聽牌 (無法聽牌)",
            tipYakumanOnly: "雖然胡牌率稍低，但可以衝擊役滿的打牌。 ({yaku})",
            tipBestAndYakuman: "聽牌張數最多且能衝擊役滿的最佳打牌！ ({yaku})"
        },

        discardResult: {
            case1Title: "[正確] 完美選擇！ (最多聽牌 & 役滿)",
            case1Desc: "打出您選擇的 [{tile}] 號牌，可在獲得最多聽牌張數(<b>{count}張</b>)的同時達成役滿聽牌！",
            case2Title: "[正確] 最佳聽牌選擇！",
            case2Desc: "打出您選擇的 [{tile}] 號牌可獲得最多的聽牌張數(<b>{count}張</b>)，胡牌率最高。",
            case3Title: "[正確] 勝率優先選擇！ (放棄役滿)",
            case3Desc: "放棄役滿機會，選擇聽牌張數最多的 [{tile}] 號牌(<b>{count}張</b>)，使勝率最大化。",
            case4Title: "[正確] 認可役滿嘗試！",
            case4Desc: "雖然聽牌張數不是最多，但您選擇了有機會一波逆轉的役滿聽牌打牌 [{tile}] 號牌！",
            case5Desc: "您選擇的 [{tile}] 號牌聽牌張數不是最多，或無法達成聽牌。",
            recMaxWait: "獲得最多聽牌的打牌: <b>[ {tiles} ]</b> (共 {count}張)",
            recYakuman: "可衝擊役滿的打牌: <b>[ {tiles} ]</b>"
        },

        alertSelectDiscardTile: "請選擇一張要打出的牌。",
    ...
    en: {
        ...
        discardReport: {
            title: "Discard Efficiency & Yakuman Analysis Report",
            tileCount: "{count} tile(s)",
            tagMaxWait: "Max Waits",
            tagYakumanTenpai: "✨ Yakuman Tenpai ({yaku})",
            discard: "Discard",
            waits: "Wait Tiles",
            totalCount: "Total {count} tile(s)",
            noten: "No-ten (Cannot reach Tenpai)",
            tipYakumanOnly: "Slightly lower win rate, but aims for a Yakuman! ({yaku})",
            tipBestAndYakuman: "The optimal discard offering max waits while aiming for Yakuman! ({yaku})"
        },

        discardResult: {
            case1Title: "[Correct] Perfect Choice! (Max Waits & Yakuman)",
            case1Desc: "Discarding tile [{tile}] yields the maximum number of waits (<b>{count} tile(s)</b>) and achieves Yakuman Tenpai!",
            case2Title: "[Correct] Optimal Discard!",
            case2Desc: "Discarding tile [{tile}] yields the maximum number of wait tiles (<b>{count} tile(s)</b>), maximizing your win rate.",
            case3Title: "[Correct] Win Rate Focused! (Yakuman Skipped)",
            case3Desc: "Skipped the Yakuman opportunity to maximize win chance with tile [{tile}] (<b>{count} tile(s)</b>).",
            case4Title: "[Correct] Recognized Yakuman Attempt!",
            case4Desc: "Although not the maximum wait count, you selected tile [{tile}] which offers a game-changing Yakuman Tenpai!",
            case5Desc: "Discarding tile [{tile}] does not yield maximum wait count or fails to achieve Tenpai.",
            recMaxWait: "Discards with maximum wait count: <b>[ {tiles} ]</b> (Total {count} tile(s))",
            recYakuman: "Discards aiming for Yakuman: <b>[ {tiles} ]</b>"
        },

        alertSelectDiscardTile: "Please select a tile to discard.",
